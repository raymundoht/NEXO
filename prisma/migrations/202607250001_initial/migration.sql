...
-- Integrity rules that Prisma cannot express in the data model.
ALTER TABLE "products"
    ADD CONSTRAINT "products_non_negative_prices"
    CHECK ("cost" >= 0 AND "sale_price" >= 0),
    ADD CONSTRAINT "products_valid_tax"
    CHECK ("tax_rate" >= 0 AND "tax_rate" <= 100),
    ADD CONSTRAINT "products_valid_stock_limits"
    CHECK ("min_stock" >= 0 AND ("max_stock" IS NULL OR "max_stock" >= "min_stock"));

ALTER TABLE "purchase_order_items"
    ADD CONSTRAINT "purchase_items_positive_quantity"
    CHECK ("quantity_ordered" > 0 AND "quantity_received" >= 0 AND "quantity_received" <= "quantity_ordered"),
    ADD CONSTRAINT "purchase_items_non_negative_cost"
    CHECK ("unit_cost" >= 0);

ALTER TABLE "purchase_receipt_items"
    ADD CONSTRAINT "receipt_items_positive_quantity"
...
ALTER TABLE "refund_items"
    ADD CONSTRAINT "refund_items_positive_values"
    CHECK ("quantity" > 0 AND "amount" >= 0);

ALTER TABLE "cash_sessions"
    ADD CONSTRAINT "cash_sessions_non_negative_opening"
    CHECK ("opening_amount" >= 0);

ALTER TABLE "sales"
    ADD CONSTRAINT "sales_non_negative_totals"
    CHECK ("subtotal" >= 0 AND "discount_total" >= 0 AND "tax_total" >= 0 AND "total" >= 0),
    ADD CONSTRAINT "sales_card_authorization_required"
    CHECK ("payment_method" <> 'CARD' OR "status" = 'HELD' OR length(trim("card_authorization")) > 0),
    ADD CONSTRAINT "sales_cash_tender_required"
    CHECK ("payment_method" <> 'CASH' OR "status" = 'HELD' OR ("amount_tendered" IS NOT NULL AND "amount_tendered" >= "total"));

ALTER TABLE "business_settings"
    ADD CONSTRAINT "business_settings_singleton"
    CHECK ("id" = 1),
    ADD CONSTRAINT "business_settings_valid_rates"
    CHECK (
        "default_tax_rate" BETWEEN 0 AND 100
        AND "max_cashier_discount_rate" BETWEEN 0 AND 100
    );

-- One user cannot keep two sessions open on the same physical register.
...
-- Emails and supplier tax identifiers are compared case-insensitively.
